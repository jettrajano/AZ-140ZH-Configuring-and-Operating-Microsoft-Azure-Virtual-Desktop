...
你需要在 Active Directory 域服务 (AD DS) 环境中配置 Windows 虚拟桌面会话主机的自动缩放功能。

## 目标
  
完成此实验室后，你将能够：

- 配置 Windows 虚拟桌面会话主机的自动缩放功能
- 验证 Windows 虚拟桌面会话主机的自动缩放功能
...
### 练习 1：配置 Windows 虚拟桌面会话主机的自动缩放功能

本练习的主要任务如下：

1. 准备自动缩放 Windows 虚拟桌面会话主机
1. 创建和配置 Azure 自动化帐户
1. 创建 Azure 逻辑应用

#### 任务 1：准备自动缩放 Windows 虚拟桌面会话主机
...
### 练习 2：验证和查看 Windows 虚拟桌面会话主机的自动缩放功能

本练习的主要任务如下：

1. 验证 Windows 虚拟桌面会话主机的自动缩放功能
1. 使用 Azure Log Analytics 跟踪 Windows 虚拟桌面事件

#### 任务 1：验证 Windows 虚拟桌面会话主机的自动缩放功能
...
#### 任务 2：使用 Azure Log Analytics 跟踪 Windows 虚拟桌面事件

>**备注**：要分析自动缩放和任何其他 Windows 虚拟桌面事件，可以使用 Log Analytics。
...
1. 在 **“查询”** 窗格左侧的 **“所有查询”** 垂直菜单中，选择 **“Windows 虚拟桌面”** 并查看预定义的查询。
